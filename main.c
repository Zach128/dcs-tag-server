...
#define DEVICE_NAME                      "DCS_Tester"                               /**< Name of device. Will be included in the advertising data. */
...
static void services_init(void)
{
    ...
    dcs_service_init(&m_dcs_service);    
}
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
    ...
    // OUR_JOB: Step 3.C, Call ble_dcs_service_on_ble_evt() to do housekeeping of ble connections related to our service and characteristic
}
...
static void advertising_init(void)
{
    ...
    uint8_t data[]                      = "DCSHost!"; // Our data to adverise
...
{   
    ...
    conn_params_init();
		...
		/*
		//Initialise FDS
		err_code = fds_dcs_init();
		APP_ERROR_CHECK(err_code);
	
		//Perform FDS test read
		char data;
		strcpy(&data, "Testing pointer returns\r\n");
		uint16_t data_size = sizeof(data) / sizeof(uint32_t);
		
		SEGGER_RTT_WriteString(0, &data);
		
		err_code = fds_dcs_overwrite((uint32_t *) &data, data_size, FDS_DCS_CIC_REC_KEY);
		APP_ERROR_CHECK(err_code);
		
		char * ret_data;
		err_code = fds_dcs_read(FDS_DCS_CIC_REC_KEY, ret_data);
		APP_ERROR_CHECK(err_code);
		
		SEGGER_RTT_WriteString(0, "Data found: ");
		SEGGER_RTT_WriteString(0, ret_data);
		SEGGER_RTT_WriteString(0, "\r\n");
		
		//dcs_cic_update(&m_dcs_service, (uint32_t)data);
		*/
